...
class DeltaReaderHttpsTest(TestCase):
    @classmethod
    def setUpClass(self):
        self.path = "tests/data/tablehttps"
        self.spark = (
            pyspark.sql.SparkSession.builder.appName("deltalake")
            .config("spark.jars.packages", "io.delta:delta-core_2.12:0.7.0")
            .config("spark.sql.extensions", "io.delta.sql.DeltaSparkSessionExtension")
        ...
        self.table = LocalDeltaReader(f"file://{self.path}")
    ...
    def test_paths(self):
        assert self.table.path == "tests/data/tablehttps"
        assert self.table.log_path == "tests/data/tablehttps/_delta_log"
